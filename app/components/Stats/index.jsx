...
const stats = [
  ...
  export default function Stats() {
    return (
      ...
      <h1 className="!z-10 mx-auto mt-2 max-w-2xl text-balance text-center text-2xl font-semibold tracking-tight text-gray-950 sm:text-5xl relative">
          ...
          <dl className="grid grid-cols-3 gap-x-8 gap-y-16 text-center">
            {stats.map((stat) => (
              <div key={stat.id} className="mx-auto flex max-w-xs flex-col gap-y-4">
                <h1 className="text-sm/5 text-gray-600 md:text-base/7">{stat.name}</h1>
                <h3 className="order-first text-4xl font-black tracking-tight text-gray-900 sm:text-6xl">
                  {stat.value}
                </h3>
              </div>
            ))}
